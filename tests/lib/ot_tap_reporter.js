(function(global) {
    ...
    function post (url, options) {
        ...
        request.send(JSON.stringify(_options.data));
    }

    function log(str) {
        try {
            post('/tests/tap', { headers: { 'Content-Type': 'application/json' },
            data: [str] });
        }
        catch(err) {
            console.error('NO CAN AJAX', str);
        }
    }
     ...
     * Usage:
     *
     * jasmine.getEnv().addReporter(new ot.TapReporter());
     */
    exportObject.OTTapReporter = function() {
        var self = this;
        self.started = false;
        self.finished = false;
        ...
        self.jasmineDone = function() {
            ...
            post('/tests/tapDone', { headers: {}, data: {} });
